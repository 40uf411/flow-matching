...
def get_image_dataset(
    ...
    elif dataset_name == "bentheimer_bin":
        crops_root = Path(root) if root else Path("/export/home/aaouf/workspace/2d_images/bentheimer_2D_dataset")
...
def save_bentheimer_bin_real_grid(
    ...
    """
    Save a grid of real bentheimer_bin crops for visual comparison.

    Args:
        output_path: Where to save the PNG.
        root: Optional dataset root overriding the default bentheimer_bin path.
...
if __name__ == "__main__":
    ...
    default_output = Path(__file__).parents[2] / "outputs" / "cfm" / "bentheimer_bin" / "real_grid.png"
    saved_path = save_bentheimer_bin_real_grid(
        output_path=default_output,
        root=None,
        grid_size=25,
        image_size=256,  # match the default bentheimer_bin training size
        crop_size=500,
    )
    print(f"Saved bentheimer_bin real grid to {saved_path}")
