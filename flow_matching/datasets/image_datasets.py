...
def get_image_dataset(
    ...
    elif dataset_name == "banderabrown_bin":
        crops_root = Path(root) if root else Path("/export/home/aaouf/workspace/2d_images/banderabrown_2D_dataset")
...
def save_banderabrown_bin_real_grid(
    ...
    """
    Save a grid of real banderabrown_bin crops for visual comparison.

    Args:
        output_path: Where to save the PNG.
        root: Optional dataset root overriding the default banderabrown_bin path.
...
if __name__ == "__main__":
    ...
    default_output = Path(__file__).parents[2] / "outputs" / "cfm" / "banderabrown_bin" / "real_grid.png"
    saved_path = save_banderabrown_bin_real_grid(
        output_path=default_output,
        root=None,
        grid_size=25,
        image_size=256,  # match the default banderabrown_bin training size
        crop_size=500,
    )
    print(f"Saved banderabrown_bin real grid to {saved_path}")
